...
@everywhere import POMDPTools: ImplicitDistribution, Deterministic, FunctionPolicy
...
@everywhere function build_mdp(
    ...
    mdp = QuickPOMDP(
        ...
        gen=function (state, state_out, rng)
            state_in, rain_state, j = state[1:num_a],
            state[(num_a + 1):(end - 1)],
            ceil(Int, state[end])
            ...
            uncertainty_sample = [
                (uncertainty_samples[j][i][1], rain_state[i]) for i in 1:length(rain)
            ]
            simulate_stage(
                subproblems[j],
                state_params_in[j],
            ...
            next_volume = DecisionRules.get_next_state(
                subproblems[j], state_params_in[j], state_params_out[j], state_in, state_out
            )
            @info "Stage t=$j" sum(state_in) sum(rain_state) sum(state_out) sum(
                next_volume
            ) r
            sp = [next_volume; rain; j+1]
            # Partial observability: policy only sees rainfall, not full state
            o = rain
            return (sp=sp, o=o, r=(-r))
        end,
        initialstate=Deterministic([initial_state; rain_state; 1.0]),
        initialobs=(s) -> Deterministic(rain_state),
...
# build the MDPs
@everywhere mdp, num_a, max_volume = build_mdp(
    case_name,
    formulation,
    num_stages;
...
𝒮_bc = BC(;
    π=ActorCritic(SG(), DoubleNetwork(QSA(), QSA())), #ActorCritic(SG(), V()), 
...
@everywhere 𝒮_reinforce = REINFORCE(
    π=GaussianPolicy(ContinuousNetwork(model, num_a), zeros(Float32, 1)),
    S=S,
    N=2000,
    ΔN=10,
    a_opt=(batch_size=1,),
...
# Solve with PPO 
@everywhere 𝒮_ppo = PPO(
    π=ActorCritic(GaussianPolicy(ContinuousNetwork(model, num_a), zeros(Float32, 1)), V()),
    S=S,
    N=10000,
    ΔN=10,
    a_opt=(batch_size=10,),
...
# # Solver with DDPG
@everywhere QSA() =
    ContinuousNetwork(Chain(Dense(22, 64, relu), Dense(64, 64, relu), Dense(64, 1)))
@everywhere 𝒮_ddpg = DDPG(;
    π=ActorCritic(ContinuousNetwork(model, num_a), QSA()), off_policy...
)
...
# # Solve with SAC
@everywhere 𝒮_sac = SAC(;
    π=ActorCritic(
        GaussianPolicy(ContinuousNetwork(model, num_a), zeros(Float32, 1)),
        DoubleNetwork(QSA(), QSA()),
    ),
    off_policy...,
)
...
function plot_learning_mod(
    ...
    if labels == :default
        labels = N == 1 ? [""] : ["Task $i" for i in 1:N]
    end
    ...
    for i in 1:length(vertical_lines)
        plot!(
            p,
            [vertical_lines[i], vertical_lines[i]],
            [vline_range...];
            color=:black,
            linewidth=i % thick_every == 0 ? 3 : 1,
            label="",
        )
    end
...
function Base.push!(
    history::Crux.ValueHistories.History{I,V}, iteration::I, value::V
) where {I,V}
...
p = plot_learning_mod(
    [𝒮_reinforce, 𝒮_ppo, 𝒮_ddpg, 𝒮_td3, 𝒮_sac, 𝒮_bc];
    title="LTHD Training Curves",
    labels=["REINFORCE", "PPO", "DDPG", "TD3", "SAC", "BC"],
...
plot!(
    ...
    tsgdr_df[:, "Step"],
    tsgdr_df[
        :, "bolivia-ACPPowerModel-h96-2024-05-18T17:06:09.485 - metrics/training_loss"
    ];
    label="TS-GDR",
    linewidth=2,
    yscale=:log10,
