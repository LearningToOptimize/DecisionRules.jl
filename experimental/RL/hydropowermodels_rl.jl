...
@everywhere function build_mdp(
    ...
    mdp = QuickPOMDP(
        ...
        gen=function (state, state_out, rng)
            ...
            # Scale tanh output [-1,1] to physical action range matching TS-GDR scale
            state_out =
                ((state_out .+ 1) ./ 2) .* (max_volume .+ rain_state .* 0.0036) ./ 10
            uncertainty_sample = [
            ...
            sp = DecisionRules.get_next_state(
...
@everywhere QSA() =
    ContinuousNetwork(Chain(Dense(34, 64, relu), Dense(64, 64, relu), Dense(64, 1)))
@everywhere V() =
    ContinuousNetwork(Chain(Dense(2*num_a+1, 64, relu), Dense(64, 64, relu), Dense(64, 1)))
@everywhere A() = ContinuousNetwork(
    Chain(Dense(2*num_a+1, 64, relu), Dense(64, 64, relu), Dense(64, num_a, tanh))
)
@everywhere SG() = SquashedGaussianPolicy(
    ContinuousNetwork(
        Chain(Dense(2*num_a+1, 64, relu), Dense(64, 64, relu), Dense(64, num_a))
    ),
    zeros(Float32, 1),
    1.0f0,
)
...
@everywhere 𝒮_ppo = PPO(
    π=ActorCritic(SG(), V()), S=S, N=10000, ΔN=10, a_opt=(batch_size=10,)
)
...
p = plot_learning_mod(
    [𝒮_reinforce, 𝒮_ppo, 𝒮_ddpg, 𝒮_td3, 𝒮_sac];
    title="LTHD Training Curves",
    labels=["REINFORCE", "PPO", "DDPG", "TD3", "SAC"],
