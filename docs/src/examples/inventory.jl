...
# A deficit penalty ``|\hat{s}_t - s_t|`` encourages the realized state to
# track the policy's prediction.
#
...
# the policy predicts ``\hat{s}_t``, the stage MIP is solved (without
# re-fixing for evaluation), and the realized net inventory is passed to
# the next stage.
...
#
# - **Base-stock (S\* = 25)**: order to bring inventory to a fixed target.
#   Classic heuristic for stochastic lot-sizing.
# - **Random (untrained)**: freshly initialized network, shows the benefit
#   of training.
#
